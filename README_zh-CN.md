...
[![使用 EdgeOne Pages 部署](https://cdnstatic.tencentcs.com/edgeone/pages/deploy.svg)](https://console.cloud.tencent.com/edgeone/makers/new?template=node-starter-agent&from=within&fromAgent=1&agentLang=typescript)
...
│   ├── _session.ts               # 会话持久化适配器（私有模块）
│   └── _tools.ts                 # EdgeOne 工具注册表（私有模块）
...
2. **`buildTools(context)`** — 从 `context.tools` 提取 EdgeOne 沙箱工具并转换为 OpenAI function calling schema
3. **`fetch` 流式请求** — 调用 OpenAI-compatible `/chat/completions`，附带工具定义
4. **工具调用循环**（最多 10 轮） — 模型返回 `tool_calls` 时，通过 `toolRegistry.execute()` 执行，将结果追加到 messages 并重新请求
5. **SSE 输出** — 依次 yield `text_delta`、`tool_called`、`done`、`error`、`ping` 事件
6. **可观测性** — `context.tracer` 提供手动埋点 span，便于追踪
