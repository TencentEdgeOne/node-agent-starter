...
│   ├── _session.ts               # Session persistence adapter (private module)
│   └── _tools.ts                 # EdgeOne tool registry (private module)
...
2. **`buildTools(context)`** — Extracts EdgeOne sandbox tools from `context.tools` and converts to OpenAI function calling schema
3. **`fetch` streaming** — Calls OpenAI-compatible `/chat/completions` with tool definitions
4. **Tool loop** (up to 10 rounds) — If model returns `tool_calls`, execute them via `toolRegistry.execute()`, append results, re-request
5. **SSE output** — Yields `text_delta`, `tool_called`, `done`, `error`, `ping` events
6. **Tracing** — `context.tracer` provides manual instrumentation spans for observability
